...
<div class="container">
	<%
		...
		Customer cust = (Customer) session.getAttribute("loginedAdmin");
					...
					<table id="table" class="table table-condensed border=0" style="table-layout:fixed; width: 800px; margin: 0px; padding: 0px;" >
							<thead>
								<tr style="width: 800px;">
									...
									%>
										<th style="width: 60px">
											<a href="../customer/addreviewlike.jsp?bookno=<%=bookNo %>&reno=<%=re.getReviewNo() %>" class="btn btn-default btn-sm" onclick="infologin(event)"><strong style="color: red; font-size: 15px;">♥</strong>
											(<%=like %>)</a>
										</th>
									<%
										if(cust == null){
									...
									<%
										}else if(cust.getCustomerNo()==re.getCustomerNo()||"Y".equals(cust.getAdmin())){
									%>
										<th style="width: 60px">
											<a href="deletereview.jsp?bookno=<%=bookNo %>&reno=<%=re.getReviewNo() %>" class="btn btn-danger btn-sm">삭제</a>
										</th>
									<%
										}
									%>
